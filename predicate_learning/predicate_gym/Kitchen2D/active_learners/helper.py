...
def get_xx_yy(expid, method, exp='pour'):
    ...
    '''
    dirnm = 'predicate_learning/predicate_gym/Kitchen2D/data/'
    fnm_prefix = os.path.join(dirnm, exp)
    initx, inity = pickle.load(open('{}_init_data_{}.pk'.format(fnm_prefix, expid), 'rb'), encoding='bytes')
    fnm = '{}_{}_{}.pk'.format(fnm_prefix, method, expid)
    xx, yy, c = pickle.load(open(fnm, 'rb'), encoding='bytes')
...
def get_func_from_exp(exp):
    ...
    '''
    if exp == 'pour':
        from predicate_learning.predicate_gym.Kitchen2D.kitchen2d.pour import Pour
        func = Pour()
    elif exp == 'scoop':
        from predicate_learning.predicate_gym.Kitchen2D.kitchen2d.scoop import Scoop
        func = Scoop()
    return func
...
def process_gp_sample(expid, flag_lk=False, is_adaptive=True, 
    ...
    from predicate_learning.predicate_gym.Kitchen2D.active_learners.active_gp import ActiveGP
...
def sample_tgmm(center, scale, n, xmin, xmax):
    ...
    prob = [np.prod(list(map(partial(truncpdf, i=i), list(range(dx)))), axis=0) for i in range(slen)]
