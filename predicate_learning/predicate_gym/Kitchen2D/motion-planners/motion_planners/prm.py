...
class Roadmap(Mapping, object):
    ...
    def __call__(self, q1, q2):
        ...
        while len(queue) != 0:
            ...
            if cv == goal:
                return retrace(cv)
            for nv, edge in list(cv.edges.items()):
...
class DistancePRM(PRM):
    ...
    def grow(self, samples):
        old_vertices, new_vertices = list(self.vertices.keys()), self.add(samples)
...
class DegreePRM(PRM):
    ...
    def grow(self, samples):
        ...
        for v1 in new_vertices:
            degree = 0
            for _, v2 in sorted([d_v2 for d_v2 in [(self.distance(v1.q, v.q), v) for v in list(self.vertices.values())] if d_v2[1] != v1 and d_v2[0] <= self.connect_distance],  # TODO - slow, use nearest neighbors
