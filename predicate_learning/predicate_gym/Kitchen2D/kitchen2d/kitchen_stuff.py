...
COLORS = {
    ...
    'water': (26, 130, 252, 255),
    'table_s': (0, 0, 200, 255),
    'table_t': (150, 0, 0, 255),
    'table': (0, 0, 0, 255),
    'block': (0, 99, 0, 255),
    'coffee': (165, 42, 42, 255),
    'cream': (225, 225, 225, 255),
    'sugar': (255, 192, 203, 255),
}
...
class guiWorld:
    def __init__(self, caption='PyBox2D Simulator', overclock=None, visualize_text=False):
        '''
        ...
        self.colors = COLORS
        self.visualize_text = visualize_text
    ...
    def draw(self, bodies, bg_color=(255, 255, 255, 1)):
        ...
        Args:
            bodies: Dict[box2d body, color name]
        ...
        pygame.event.get()
        for body, color_name in bodies.items():
            for fixture in body.fixtures:
                fixture.shape.draw(body, fixture, color=color_name)
        
        if self.visualize_text:
            font = pygame.font.SysFont(None, 20)
            for body in bodies:
                if body.userData == 'gripper':
                    continue
                position = (self.screen_origin + body.position + (0,4)) * PPM
                position = (position[0], SCREEN_HEIGHT_PX-position[1])
                img = font.render(body.userData, True, (255,0,0))
...
class b2WorldInterface(object):
    def __init__(self, do_gui=True, save_fig=False, caption='PyBox2D Simulator', overclock=None, visualize_text=False):
        '''
        ...
        Args:
            ...
            visualize_text: visualize userData of objects
        '''
        self.world = b2World(gravity=GRAVITY, doSleep=True)
        self.do_gui = do_gui
        if do_gui:
            self.gui_world = guiWorld(caption=caption, overclock=overclock, visualize_text=visualize_text)
        else:
            self.gui_world = None
    ...
    def enable_gui(self, caption='PyBox2D Simulator'):
        ...
        if self.gui_world is None:
            self.gui_world = guiWorld(caption=caption, overclock=self.overclock, visualize_text=self.visualize_text)
    ...
    def draw(self):
        ...
        if not self.do_gui:
            return
        bodies_to_draw = {body: body.userData for body in self.world.bodies}
        bodies_to_draw.update(self.body_to_color)
        self.gui_world.draw(bodies_to_draw)
...
class Liquid(object):
    def __init__(self, world, liquid_frequency=0.2, density=0.01, friction=0.0, radius=0.1, shape_type='circle', liquid_name='water'):
        '''
...
def make_block(b2world_interface, pos, angle, w, h, user_data="block", d=None):
    '''
