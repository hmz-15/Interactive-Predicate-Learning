...
def create_gripper(
    ...
    lgripper = world.CreateDynamicBody(
        position=lgripper_pos, angle=init_angle, gravityScale=1.0, allowSleep=True
    )
    rgripper = world.CreateDynamicBody(
        position=rgripper_pos, angle=init_angle, gravityScale=1.0, allowSleep=True
    )
...
def get_gripper_lrpos(init_pos, init_angle, width):
    ...
    lgripper_pos = init_pos - width / 2.0 * b2Vec2((np.cos(init_angle), np.sin(init_angle)))
    rgripper_pos = init_pos + width / 2.0 * b2Vec2((np.cos(init_angle), np.sin(init_angle)))
...
def incup(cup, particles, p_range=SCREEN_WIDTH / 2, debug=0):
    ...
    for p in particles:
        ...
        if ppos[0] <= cup.usr_w and ppos[0] >= 0.0 and ppos[1] >= 0.0 and ppos[1] <= cup.usr_h:
            ...
            outcupparticles += [p]
        if (
            (np.abs(p.linearVelocity[1]) > 0.01)
            and np.abs(p.position[0]) < p_range
...
def buffer_shape(shape, radius, sides=4):
    ...
    thetas = [(i + 0.5) * 2 * np.pi / sides for i in range(sides)]
        ...
        for fixture in b.fixtures:
            if buffer_radius == 0:
                ...
                obstacle.CreateFixture(
                    shape=buffer_shape(fixture.shape, buffer_radius), isSensor=True
                )
    return b2w
...
class Gripper(object):
    def __init__(
        ...
        if lgripper_pos is None or rgripper_pos is None:
            ...
            init_pos = b2Vec2(init_pos)
            lgripper_pos, rgripper_pos = get_gripper_lrpos(
                init_pos, init_angle, init_width + self.gripper_width
            )

        self.lgripper, self.rgripper, self.pj = create_gripper(
            world,
            lgripper_pos,
            rgripper_pos,
            init_angle,
        ...
        self.laerror_integral = 0.0
        self.rerror_integral = b2Vec2((0, 0))
        self.raerror_integral = 0.0
        self.attached = False
    ...
    def pour(
        ...
        found = self.find_path(dpos, 0.0, maxspeed=maxspeed)
        print("found", found)
        if not found:
            return False, 0
        if self.planning:
    ...
    def get_cup_feasible(self, from_obj):
        ...
        if self.attached_obj.userData == "spoon":
            offset_l = GRIPPER_WIDTH + self.attached_obj.usr_w / 2.0
            offset_r = np.max((self.attached_obj.usr_d, offset_l))
        else:
            offset_l = GRIPPER_WIDTH + self.attached_obj.usr_w / 2.0
            offset_r = offset_l
        # position of the left lower corner of the cup inside
        cup_left_offset = from_obj.position + np.array(
            [
                -from_obj.usr_w / 2.0 + from_obj.usr_d + offset_l + EPS,
                from_obj.usr_d + EPS,
            ]
        )

        # feasible range of spoon tip positions relative to cup_left_offset
        cup_feasible = np.array(
            [
                from_obj.usr_w - from_obj.usr_d * 2 - offset_r - offset_l - EPS * 2.0,
    ...
    def get_dump_init_end_pose(self, to_obj, rel_pos_x):
        ...
        pos_x = delta_pos - to_obj.usr_w / 2.0 + rel_pos_x * to_obj.usr_w
        dpos1 = to_obj.position + (pos_x, to_obj.usr_h + spoon_d + 1.0)

        dpos2 = dpos1.copy()
        dpos2[0] -= delta_pos
        dpos2[1] += delta_pos

        return np.hstack((dpos1, -np.pi / 2)), np.hstack((dpos2, 0.0))
    ...
    def get_scoop_init_end_pose(self, from_obj, rel_pos1, rel_pos3):
        ...
        horizontal_offset = np.array([delta_pos, 0.0])

        # initial position of scoop
        dpos1 = cup_left_offset + np.array(rel_pos1) * cup_feasible + verticle_offset
        dpos3 = cup_left_offset + horizontal_offset + np.array(rel_pos3) * cup_feasible

        dpos = dpos3.copy()
        dpos[1] = from_obj.position[1] + from_obj.usr_h + self.attached_obj.usr_d + 1.0

        return np.hstack((dpos1, 0.0)), np.hstack((dpos, -np.pi / 2))
    ...
    def scoop(self, from_obj, rel_pos1, rel_pos2, rel_pos3, maxspeed=1.0):
        ...
        found = self.find_path(dpos1, 0.0)
        if not found:
            return False, 0
        dpos2 = cup_left_offset + np.array(rel_pos2) * cup_feasible + verticle_offset

        dpos3 = cup_left_offset + horizontal_offset + np.array(rel_pos3) * cup_feasible

        traj = np.vstack((np.hstack((dpos2, 0.0)), np.hstack((dpos3, -np.pi / 2))))
        ...
        return True, (len(in_spoon) + outside) / (
            0.5 * self.attached_obj.usr_d**2 * np.pi / (self.b2w.liquid.radius**2 * 4)
        )
    ...
    def get_liquid_from_faucet_pos(self):
        ...
        if (
            ...
            - self.attached_obj.position[1]
            - self.attached_obj.usr_h
            > 0
        ):
    ...
    def get_liquid_from_faucet(self, runtime, maxspeed=1.0):
        """
        ...
        found = self.find_path(gripper_pos, 0, maxspeed=maxspeed)
        if not found:
            return False
        if self.planning:
            return True
    ...
    def get_stir_init_end_pose(self, from_obj, rel_pos1, rel_pos2):
        ...
        dpos1 = cup_left_offset + np.array(rel_pos1) * cup_feasible + vertical_offset
        # ending position of stir
        dpos2 = cup_left_offset + np.array(rel_pos2) * cup_feasible + vertical_offset
        return np.hstack((dpos1, 0.0)), np.hstack((dpos2, 0))
    ...
    def stir(self, obj, rel_pos1, rel_pos2, num_stirs=5):
        ...
        for i in range(num_stirs):
            self.apply_lowlevel_control(dposa1[:2], dposa1[2], maxspeed=1.0)
            self.apply_lowlevel_control(dposa2[:2], dposa2[2], maxspeed=1.0)
        return True
    ...
    def grasp_translation(self, body, pos_ratio):
        ...
        if body.userData == "spoon" or body.userData == "stir":
            grasp_w = (
                body.usr_d * 2 + GRIPPER_HEIGHT / 2 + EPS + (body.usr_h * 0.9 - 2 * EPS) * pos_ratio
            )
            gripper_pos = body.position + grasp_w * np.array(
                [-np.sin(body.angle), np.cos(body.angle)]
            )
        else:
            # assert(np.isclose(body.angle, 0, atol=1e-2))
            lower, upper = self.bounding_box(body)
            body_h = (upper[1] - lower[1]) - EPS
            gripper_pos = np.array(
                [
                    body.position[0],
                    lower[1] + (pos_ratio * body_h) + (self.gripper_height / 2) + (EPS / 2.0),
                ]
            )
    ...
    def get_grasp_poses(self, body, pos_ratio):
        ...
        dpos_up = np.array([dpos[0], upper[1] + 1.0 + (self.gripper_height / 2)])
        return dpos, dpos_up

    def place(self, obj_dpos, obj_dangle, maxspeed=1.0):
        """
        ...
        dpos_gripper[1] += EPS
        found = self.find_path(dpos_gripper, obj_dangle, collision_buffer=0.1, maxspeed=maxspeed)
        ...
        self.apply_lowlevel_control(dpos_up, 0, maxspeed=maxspeed)
        return True
    ...
    def grasp(self, body, pos_ratio, maxspeed=1.0):
        """
        Grasp a Box2D body.
        Args:
            body: a Box2D body to be grasped.
        ...
        # test in simulation: when gripper is open, whether collide when at dpos_up
        g2 = self.simulate_itself(collision_buffer=0.1)
        g2.open()
        col = g2.check_point_collision(dpos_up, 0)
        if col:
            return False

        # traj = self.plan_path(dpos_up, 0)
        # if traj is None:
        #     return False

        # close gripper and navigate to dpos_up
        self.close()
        found = self.find_path(dpos_up, 0, collision_buffer=0.1, maxspeed=maxspeed)
        if not found:
            return False
        # open gripper and navigate to dpos
        self.open()
        found = self.find_path(dpos, 0, maxspeed=maxspeed)
        if not found:
            return False
        self.close()
    ...
    def set_closed(self):
        ...
        self.lgripper.position, self.rgripper.position = get_gripper_lrpos(
            self.position, self.angle, width
        )
        self.pj.motorSpeed = -MOTOR_SPEED
    ...
    def get_push_pos(self, body, goal_x):
        ...
        y = lower[1] + self.gripper_height / 2.0 + ground_offset
    ...
    def push(self, body, rel_pos, goal_x, reach_maxspeed, maxspeed):
        """
        Push an object.
        Args:
            body: a Box2D body to be pushed.
        ...
        for _ in range(5):
            lower, upper = self.bounding_box(body)
            dpos = lower + rel_pos + self.gripper_height / 2.0 + 0.2
            if goal_x < body.position[0]:
                dpos[0] = upper[0] + rel_pos[0] + self.gripper_width
            else:
                dpos[0] = lower[0] - rel_pos[0] - self.gripper_width
            self.close()
            found = self.find_path(dpos, 0, maxspeed=reach_maxspeed)
            if not found:
                print(found)
                return False
            if self.planning:
                return True
            if goal_x < body.position[0]:
                dpos[0] = goal_x + body.usr_w / 2.0 + self.gripper_width
            else:
                dpos[0] = goal_x - body.usr_w / 2.0 - self.gripper_width
            ...
            dangle = 0
            # adjust_cnt = 0
            # if posa_metric(self.position, self.angle, dpos, dangle) > 0.01:
    ...
    def compute_post_grasp_mass(self, p_range=SCREEN_WIDTH):
        ...
        stopped = True
        if obj.userData == "cup":
            incupparticles, outcupparticles, stopped = incup(
                obj, self.b2w.liquid.particles, p_range=p_range
            )
    ...
    def apply_dmp_control(self, traj, maxspeed=0.5, a=1.0, b=0.5):
        """
        ...
        segment_lengths = np.array(
            [0.0] + [np.linalg.norm(traj[i] - traj[i - 1]) for i in range(1, len(traj))]
        )
        ...
        path_gen = scipy.interpolate.interp1d(
            np.cumsum(segment_lengths) * runtime / tot_length,
            traj.T,
        ...
        for step in range(timesteps + 2):
            ...
            if self.attached:
                aforce += (
                    (-GRAVITY[1])
                    * self.attached_obj.mass
                    * (self.attached_obj.worldCenter[0] - self.position[0])
                    / self.radius
                )
    ...
    def apply_lowlevel_control(self, dpos, dangle, maxspeed=0.5, collision_check=False):
        """
    ...
    def check_point_collision(self, pos, angle=0.0):
        """
    ...
    def check_path_collision(self, pos, angle, dpos, dangle):
        ...
        """
        self.set_position(pos, angle)
        traj, collision = self.apply_lowlevel_control(
            dpos, dangle, maxspeed=1.0, collision_check=True
        )
        if not collision:
    ...
    def check_collision(self):
        ...
        for c in contacts:
            if c.contact.touching and (
                c.contact.fixtureA.body.userData is "obstacle"
    ...
    def set_position(self, pos, angle):
        ...
        if self.attached:
            ...
            p = np.array([[np.cos(rel_a), -np.sin(rel_a)], [np.sin(rel_a), np.cos(rel_a)]]).dot(p)
    ...
    def info(self):
        return (
            self.lgripper.position,
            self.lgripper.angle,
            self.rgripper.position,
            self.rgripper.angle,
            self.pj.motorSpeed,
        )
    ...
    def simulate_itself(self, collision_buffer=None):
        ...
        if self.attached:
            g2.attached = True
            # find attached object name
            obj = None
            for name, obj in self.b2w.objects.items():
                if obj == self.attached_obj:
                    break
            assert obj is not None, "Unknown attached object!"
            # attached_obj_pose = np.hstack((self.attached_obj.position, self.attached_obj.angle))
            # attached_obj_size = {
            #     name: (
            #         self.attached_obj.usr_w,
            #         self.attached_obj.usr_h,
            #         self.attached_obj.usr_d,
            #     )
            # }
            g2.attached_obj, _, _ = self.b2w.create_object(
                world=b2w2.world,
                name=name,
                pos=self.attached_obj.position,
            )
    ...
    def plan_path(self, dpos, dangle, collision_buffer=None, motion_angle=0):
        ...
        traj = motion_planner(
            ...
            dpos,
            dangle,
            g2.check_point_collision,
            g2.check_path_collision,
    ...
    def gripper_state(self):
        # gripper_pos = np.array([self.position[0] / SCREEN_WIDTH, self.position[1] / SCREEN_HEIGHT])
        gripper_pos = np.array(self.position)
        gripper_open_width = (
            self.rgripper.position[0] - self.lgripper.position[0] - self.gripper_width
        )
        gripper_open_width = np.clip(gripper_open_width, 0.0, OPEN_WIDTH)

        min_l, max_l = self.bounding_box(self.lgripper)
        min_r, max_r = self.bounding_box(self.rgripper)

        min_all = np.min([min_l, min_r], axis=0)
        max_all = np.max([max_l, max_r], axis=0)

        # convert bbox into image coordinate (X, Y, W, H)
        # Box2D gui_world coordinate is at (SCREEN_WIDTH/2., TABLE_HEIGHT)
        img_bbox = np.array(
            [
                (SCREEN_WIDTH / 2 + min_all[0]) * PPM,
                (SCREEN_HEIGHT - (TABLE_HEIGHT + max_all[1])) * PPM,
                (max_all[0] - min_all[0]) * PPM,
                (max_all[1] - min_all[1]) * PPM,
            ]
        )

        world_bbox = np.array([min_all, max_all]).reshape(-1)

        ingripper_mass = (self.mass - self.lgripper.mass - self.rgripper.mass) / 1.0
        ingripper_mass = 0 if ingripper_mass < 0 else ingripper_mass
        # print(ingripper_mass)
        return {
            "position": gripper_pos,
            "open_width": gripper_open_width,
            "in_gripper_mass": ingripper_mass,
            "height": self.gripper_height,
            "max_open_width": OPEN_WIDTH,
        }
