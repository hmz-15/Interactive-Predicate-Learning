...
from . import place_holders as ph
import numpy as np
...
def genScoop(grip_name, spoon_name, kettle_name, pose2):
    ...
    while True:
        ...
        scoop_pose, end_pose = list(map(tuple, gripper.get_scoop_init_end_pose(kettle, rel_pos1, rel_pos3)))
...
def genDump(grip_name, spoon_name, grasp, kettle_name, pose2):
    ...
    gripper.set_grasped(spoon, grasp, ph.gripperInitPos[:2], ph.gripperInitPos[2])
    rel_pos_x = 0.8
    init_pose, dump_pose = list(map(tuple, gripper.get_dump_init_end_pose(kettle, rel_pos_x)))
...
def genStir(grip_name, stir_name, grasp, kettle_name, pose2):
    ...
    init_pose, stir_pose = list(map(tuple, gripper.get_stir_init_end_pose(kettle, (0, 0), (1, 0))))
