import sys
sys.path.append('predicate_learning/predicate_gym/Kitchen2D/pddlstream/')
...
def sample_task_instance():
    ...
    def sample_from_region(region):
        weight = [abs(np.prod(a[1]-a[0])) for a in region]
...
def solve_task(kitchen_params, initial_poses, object_sizes, expid_pour, expid_scoop):
    ...
    input('Start running STRIPStream solver?')
    ...
    if input('Show plan? [y/n]') in ['y', 'Y']:
        # Show plan simulation
        step_plan(plan, kitchen, body_from_name)
    if input('Show plan execution? [y/n]') in ['y', 'Y']:
